...
import { Settings, ChevronLeft, Sliders, Loader2, PanelLeftClose, PanelLeft } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!sourcesError) {
          setSources(
            allSources?.map((s) => ({ id: s.id, name: s.name, type: s.source_type, chunks: s.chunk_count })) || []
          );
        }
          ...
          >
            <div className="p-6 space-y-6 h-full overflow-auto">
              <div>
                ...
                <div className="mb-6">
                  <label className="text-sm font-medium text-muted-foreground mb-3 block">Assistant Mode</label>
                ...
                <div className="glass rounded-lg p-4 mb-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs text-muted-foreground block mb-2">Top K Results: 5</label>
                      <input type="range" min="1" max="20" defaultValue="5" className="w-full accent-primary" />
                    </div>
                    <div>
                      <label className="text-xs text-muted-foreground block mb-2">Similarity Threshold: 0.7</label>
                      <input type="range" min="0" max="100" defaultValue="70" className="w-full accent-primary" />
                    </div>
                  </div>
                </div>
                <SourceSelector sources={sources} selectedIds={selectedSourceIds} onToggle={toggleSource} />
                {sources.length === 0 && (
